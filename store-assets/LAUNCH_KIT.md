...
The v0.9.5 candidate is macOS-first. Chrome Developer mode remains necessary until a Chrome Web Store release is approved. Core setup, selection saving, First Evidence, and Replay controls are available in English and Simplified Chinese; advanced workspaces remain in Simplified Chinese. A right-click save persists one exact selection as pending local evidence without a model call, and a person must still accept or reject it. Replay verifies the local captured snapshot and does not promise a precise position on the current remote page. Complex PDFs and sites still need real-world testing.
...
The product is a Chrome side panel plus a local Python Companion. It captures pages in your live browser session, imports PDFs and public captions, and stores the durable record as SQLite plus readable Markdown. Quick Start can create a first draft claim with an exact stored-source quote through deterministic local template extraction. That draft is not an AI summary and remains unreviewed until the user marks it supported or unsupported. Model use is optional: Codex CLI or an OpenAI-compatible or Anthropic endpoint.

v0.9.5 also adds an explicit context-menu action for one selected passage. It saves the exact quote plus bounded captured context as pending local evidence without calling a model or agent; a person still decides whether to accept or reject it. Failed or unpaired saves remain in a bounded `chrome.storage.local` recovery queue until the Companion write succeeds and the extension ACK removes the item. The queue is not telemetry and is not sent to the developer.

This is a v0.9.5 public preview. It is macOS-first, Chrome Developer mode is required until a Chrome Web Store release is approved, and advanced workspaces remain in Simplified Chinese. Settings links directly to the matching Companion ZIP and checksum file, while core setup, selection saving, First Evidence, and Replay controls are available in English and Simplified Chinese. I would especially value reports from people who try the selection-first or zero-configuration First Evidence path, replay an older citation, process a difficult forum thread, or import a real paper. Which step feels least trustworthy or most cumbersome?

### Gallery captions

1. **Capture with context** — explicitly save one selection as pending local evidence, read the current page, or recover a URL batch in the Chrome side panel.
2. **Review the evidence** — compare every claim with its exact quote and source context before marking it supported or unsupported.
...
- **Why broad HTTP(S) host access?** To extract arbitrary HTTP(S) pages only after the user chooses them and to reach the loopback Companion over HTTP; file and other URL schemes are not requested, and there is no passive history collection.
...
v0.9.5 adds an explicit right-click action that saves one selected exact quote plus bounded captured context as pending local evidence. It makes no model or agent call and cannot accept or reject the evidence on the user's behalf. Failed or unpaired saves remain in a bounded Chrome-local recovery queue until Companion persistence and ACK succeed. The release also carries Source Replay forward and retains direct Settings links to the exact version-matched Companion ZIP and `SHA256SUMS`. Saved evidence can be reopened against its captured context, locator, source version, and exact quote, with explicit resolved, stale, or unresolved states. The product can also import PDFs with page citations, use macOS Vision OCR for low-text pages, ingest public/manual YouTube captions, recover long URL batches, and produce evidence-backed research outputs. Model use is optional; deterministic local template extraction can exercise the evidence workflow without pretending to be an AI summary, and every draft still needs a human supported/unsupported decision.

Repository and release: https://github.com/vyang472/qc-smart-reader-extension

The honest limitations are macOS-first packaging, Simplified Chinese advanced workspaces, Developer-mode installation, and incomplete complex-PDF handling. Core setup and First Evidence are bilingual. If this matches your workflow, I would value a real test and a blunt report about where trust or usability breaks.

## X / Mastodon draft

I built QC Smart Reader: an open-source, local-first Chrome reader where a claim cannot become “reviewed” unless its exact quote still exists in the source. v0.9.5 adds an explicit zero-model right-click path from one selected passage to pending local evidence, with human accept/reject and Source Replay. Web/PDF/public captions → evidence graph → Markdown + SQLite Vault. Bilingual core path, macOS-first: https://github.com/vyang472/qc-smart-reader-extension/releases/tag/v0.9.5

## LinkedIn draft

I have released QC Smart Reader v0.9.5, an open-source Chrome extension and local Companion for research that needs an audit trail.
...
The public preview supports an explicit, zero-model selection-first path: right-click one selected passage to save its exact quote plus bounded captured context as pending local evidence, then accept or reject it yourself. Failed or unpaired saves remain in a bounded Chrome-local recovery queue until local Companion persistence and ACK succeed; the queue is not telemetry or a model input. It also supports a zero-configuration whole-page First Evidence path, Source Replay, restart-safe URL batches, PDF page citations with macOS OCR, public/manual YouTube captions, local Markdown + SQLite storage, and optional Codex CLI or API providers. Its local template creates only an unreviewed draft with a real quote; it does not impersonate an AI summary.

It is intentionally honest about its current edges: macOS-first, Simplified Chinese advanced workspaces, Developer-mode Chrome installation, and more work needed on complex PDFs. Core setup, First Evidence, and Replay controls are available in English and Simplified Chinese.
...
QC Smart Reader v0.9.5: save one exact selection without a model
...
- explicit right-click selection saving as pending local evidence, with zero model or agent calls and a required human accept/reject decision;
- direct Settings links to the version-matched Companion ZIP and `SHA256SUMS`;
- Source Replay from the captured context, locator, source version, and exact quote, with fail-closed resolved, stale, and unresolved states;
- pairing-gated Quick Start that persists the current page and waits for a human supported/unsupported decision on its local-template draft;
- browser-locale-aware English / Simplified Chinese core onboarding with a local language selector;
- local SQLite + readable Markdown Vault;
- current-page, selection-first pending evidence, and restart-safe batch capture;
